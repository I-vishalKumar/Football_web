...
function bet_amount(){
  ...
  let pr = parseFloat(prof1[0].innerText);
  let ammount = parseFloat(am1.value);

  if(isNaN(pr/100 * ammount)){
    exp_prof.innerText = "";
  }else{

    exp_prof.innerText = parseFloat((pr/100 * ammount).toFixed(2));

    console.log(`profit i got ${pr} after calculation ${exp_prof.innerText}`);
  }
...
conf.addEventListener('click' , function(e){
  ...
  const data = {
     ...
     profit :     parseFloat(prof[0].innerText),
  ...
  xmlHttp.onreadystatechange = function(res) {
      ...
      if (xmlHttp.readyState === 4) {
         ...
         res = JSON.parse(xmlHttp.responseText);
          ...
          // 0 = low balance , 1 = bet done , 2 = bet already exits , -1 = someting went wrong,5 = bet ammount is smaller than 500
            ...
            if(res.status === 1){
              ...
              res_text = "Minimum bet ammount is 500";
              res_background = "#ffc6c6";
            }
